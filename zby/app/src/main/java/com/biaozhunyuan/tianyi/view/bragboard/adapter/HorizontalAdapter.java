...
import static com.biaozhunyuan.tianyi.view.bragboard.helper.DragHelper.TYPE_CONTENT;
import static com.biaozhunyuan.tianyi.view.bragboard.helper.DragHelper.TYPE_FOOTER;
...
//        notifyItemChanged(position);
    ...
    public void appendNewColumn(DragColumn dragColumn,RecyclerView recyclerView) {
        ...
        if(getData().size() == 1 || getData().size() == 2){ //如果列表最初没有列 新建之后在当前列
            ViewHelper.setRecycleViewLRPadding(mContext,recyclerView,10,70);
        } else if (getData().size() > 2){ //如果列表最初有一列 新建之后在中间列
            ViewHelper.setRecycleViewLRPadding(mContext,recyclerView,40,40);
        }

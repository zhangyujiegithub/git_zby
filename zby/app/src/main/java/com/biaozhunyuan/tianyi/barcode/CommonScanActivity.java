...
//            rescan.setVisibility(View.INVISIBLE);
    ...
    private void getProductInfoAndBalance(Inventory inventory) {
        String url = Global.BASE_JAVA_URL + GlobalMethord.进销存商品详情和库存查询 + "?skuId=" + inventory.getUuid();
    ...
    private void getCargoInfo(Inventory inventory) {
        String url = Global.BASE_JAVA_URL + GlobalMethord.进销存商品详情查询 + "?barcode=" + inventory.getUuid();
        StringRequest.getAsyn(url, new StringResponseCallBack() {
            @Override
            public void onResponse(String response) {
                ...
                try {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        } else if (key.equals("purchasePrice") && (formName.contains("入库") || formName.contains("调拨"))) {
                            map.put(key, value);
                            map.put("单价", value);
                        } else if (key.equals("price") && formName.contains("出库")) {
                            map.put(key, value);
                            map.put("单价", value);

...
public class ClientNewContactWayActivity extends BaseActivity {
    ...
    private void getIntentData() {
        if (getIntent().getStringExtra("id") != null) {
            ...
            boolean isCanSave = getIntent().getBooleanExtra("isCanSave", false);
            headerview.setRightTitleVisible(isCanSave);
            headerview.setTitle("联系人详情");
            getList(id);
        } else if(getIntent().getStringExtra("customerId")!=null){
            clientId = getIntent().getStringExtra("customerId");
            getList("0");
        }
    }
    ...
    private void getList(String id) {
        ...
        try {
            object.put("type", "crm_customer_contact");
            object.put("id", id);
                            ...
                            while (it.hasNext()) {
                                ...
                                if (keyStr.equals(title)) {
                                    ...
                                    ChClientTabFragment fragment = ChClientTabFragment
                                            .newInstance(formList, true,"crm_customer_contact",id);
...
//                        result = ChClientBiz.checkRegEx(list);
    ...
    private void saveCustomerForm(ArrayList<表单字段> formList) {
        ProgressDialogHelper.show(ClientNewContactWayActivity.this);
        String url = Global.BASE_JAVA_URL + GlobalMethord.保存动态字段;
        StringRequest.postAsynNoMap(url, "crm_customer_contact", formList, new StringResponseCallBack() {
            @Override

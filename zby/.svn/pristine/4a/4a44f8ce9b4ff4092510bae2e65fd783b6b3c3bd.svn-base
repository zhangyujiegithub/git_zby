 ...
 * 表单选择工程出差报告
 */
public class FormSelectProjectNumberActivity extends BaseActivity {

    private BoeryunHeaderView headerview;
    private ListView lv;
    private List<Project> list = new ArrayList<>();
    private CommanAdapter<Project> adapter;
    public static final String REQUEST_SELECT_PROJECT_FORMNUMBER = "REQUEST_SELECT_PROJECT_FORMNUMBER";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_form_select_project_number);
        initView();
    ...
    private void initDemand() {
        ...
        demand.dictionaryNames = "projectId.crm_project";
        demand.src = url;
    }
...
//                adapter.notifyDataSetChanged();
    ...
    private CommanAdapter<Project> getAdapter(List<Project> gridItems) {
        return new CommanAdapter<Project>(gridItems, this, R.layout.item_form_select_project_list) {
            public void convert(int position, Project item, BoeryunViewHolder viewHolder) {
                viewHolder.setTextValue(R.id.tv_productname, item.getReporterName()); //报告人
                viewHolder.setTextValue(R.id.tv_producttype, item.getProjectName() + "");//项目名称
                viewHolder.setTextValue(R.id.tv_productunit, item.getCompanyName()); //公司名称
                viewHolder.setTextValue(R.id.tv_productprice, item.getTravelContent());//出差内容
            }
        };
    }
    ...
    private void returnResult(List<Project> list) {
        ...
        bundle.putSerializable(REQUEST_SELECT_PROJECT_FORMNUMBER, projectList);

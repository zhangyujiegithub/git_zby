...
import com.biaozhunyuan.tianyi.client.ClientBusinessListFragment;
import com.biaozhunyuan.tianyi.client.ClientlistbusinessFragment;
 ...
 * 商机信息
 */
public class BusinessInfoActivity extends BaseActivity implements BDLocationListener {
...
//        getList();
    ...
    private void requestLocating() throws Exception {
        BaiduLocator.requestLocation(getApplicationContext(), BusinessInfoActivity.this);
    }
    ...
    private void getList() {
        ...
        try {
            ...
            object.put("id", mBusiness.getUuid());
...
//                                    }
                                    ...
                                    ChClientTabFragment fragment = ChClientTabFragment
                                            .newInstance(formList,mBusiness.isCanSave(),"crm_sale_chance",mBusiness.getUuid());
...
//                        mIndicator.setVisibility(View.GONE);
    ...
    private void initView() {
        ...
        headerview.setRightTitleVisible(mBusiness.isCanSave());
        headerview.setOnButtonClickListener(new BoeryunHeaderView.OnButtonClickRightListener() {
            @Override
            public void onRightTextClick() {
                try {
                    String add_business_lat = PreferceManager.getInsance().getValueBYkey("add_business_lat" + mBusiness.getUuid());
                    String add_business_lng = PreferceManager.getInsance().getValueBYkey("add_business_lng" + mBusiness.getUuid());
                    mLatitude = Double.parseDouble(add_business_lat);
                    mLongitude = Double.parseDouble(add_business_lng);
                }catch (Exception e){
                    e.printStackTrace();
                }
                String url = Global.BASE_JAVA_URL + GlobalMethord.商机地址校验;
                Business business = new Business();
                business.setJing(mLongitude + "");
                business.setWei(mLatitude + "");
                business.setUuid(mBusiness.getUuid());
                StringRequest.postAsyn(url, business , new StringResponseCallBack() {
                    @Override
...
//        info.setType("crm_customer");
        ...
        ProgressDialogHelper.show(BusinessInfoActivity.this);
        String url = Global.BASE_JAVA_URL + GlobalMethord.保存动态字段;
        StringRequest.postAsynNoMap(url, "crm_sale_chance", formList, new StringResponseCallBack() {
            @Override
            public void onResponseCodeErro(String result) {
                ProgressDialogHelper.dismiss();
    ...
    private void saveLngLat(String data) {
        Business business = new Business();
        business.setLng(mLongitude+"");
        business.setLat(mLatitude+"");
        business.setUuid(data);
        StringRequest.postAsyn(Global.BASE_JAVA_URL + GlobalMethord.保存商机经纬度, business, new StringResponseCallBack() {
            @Override
            public void onResponse(String response) {
                showShortToast("保存成功");
                setResult(RESULT_OK);
                ClientlistbusinessFragment.isResume = true;
                BusinessListActivity.isResume = true;
                ClientBusinessListFragment.isResume = true;
                finish();
            }
...
//                    }
                    ...
                    if(place.location !=null) {
                        ...
                        PreferceManager.getInsance().saveValueBYkey("add_business_lat" + mBusiness.getUuid(), lat);
                        PreferceManager.getInsance().saveValueBYkey("add_business_lng" + mBusiness.getUuid(), log);
                    }
                }
            }
        }

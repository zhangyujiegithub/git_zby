package com.biaozhunyuan.tianyi.view.bragboard;
...
import com.biaozhunyuan.tianyi.view.bragboard.adapter.RecyclerViewPagerAdapter;
import com.biaozhunyuan.tianyi.view.bragboard.model.DragColumn;
import com.biaozhunyuan.tianyi.view.bragboard.utils.RecyclerviewUtils;
...
public class PagerRecyclerView extends RecyclerView {
    ...
    private List<DragColumn> mDataSize;
    ...
    public void setAdapter(Adapter adapter, RecyclerView recyclerView, List<DragColumn> dataList , Context context) {
        this.mContext = context;
        this.mRecycleView = recyclerView;
        this.mDataSize = dataList;
        width = ViewHelper.px2dip(mContext, ViewHelper.getScreenWidth(mContext) / 3);
        midWidth = width / 2 + 10;
    ...
    public void smoothScrollToPosition(int position) {
        ...
        if(mDataSize.size() == 0){
            ViewHelper.setRecycleViewLRPadding(mContext,mRecycleView,10,10);
        } else {
            if(position == mDataSize.size()){ //如果是最后一列
                ViewHelper.setRecycleViewLRPadding(mContext,mRecycleView,width,10);
            } else if(position == 0){//如果是第一列
                ViewHelper.setRecycleViewLRPadding(mContext,mRecycleView,10,width);
            } else {
                ViewHelper.setRecycleViewLRPadding(mContext,mRecycleView,midWidth,midWidth);
            }
        }

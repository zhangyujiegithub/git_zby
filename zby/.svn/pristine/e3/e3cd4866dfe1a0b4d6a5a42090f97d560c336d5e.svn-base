 ...
 * 新建商机
 */
public class BusinessAddActivity extends BaseActivity implements BDLocationListener {
    ...
    private void requestLocating() throws Exception {
        BaiduLocator.requestLocation(getApplicationContext(), BusinessAddActivity.this);
    }
                            ...
                            while (it.hasNext()) {
                                ...
                                if (keyStr.equals(title)) {
                                    ...
                                    ChClientTabFragment fragment = ChClientTabFragment
                                            .newInstance(formList, true,"crm_sale_chance","0");
...
//                        result = ChClientBiz.checkRegEx(list);
    ...
    private void saveCustomerForm(ArrayList<表单字段> formList) {
        ProgressDialogHelper.show(BusinessAddActivity.this);
     ...
     * @param data
     */
    private void saveLngLat(final String data) {
        Business business = new Business();
        ...
        StringRequest.postAsyn(Global.BASE_JAVA_URL + GlobalMethord.保存商机经纬度, business, new StringResponseCallBack() {
            @Override
            public void onResponse(String response) {
                PreferceManager.getInsance().saveValueBYkey("add_business_lat" + data, mLatitude+"");
                PreferceManager.getInsance().saveValueBYkey("add_business_lng" + data, mLongitude+"");
                showShortToast("保存成功");
                setResult(RESULT_OK);
...
//            BaiduLocator.stop();
            ...
            //实例化一个当前定位到的地址对象
            BaiduPlace mBaiduPlace = new BaiduPlace();

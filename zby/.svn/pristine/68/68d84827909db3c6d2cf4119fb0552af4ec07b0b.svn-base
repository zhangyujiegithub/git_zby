...
public class DragBoardView extends DragLayout {
    ...
    public void setHorizontalAdapter(@NonNull HorizontalAdapter horizontalAdapter) {
        ...
        mRecyclerView.setAdapter(mAdapter,mRecyclerView , mAdapter.getData(),mContext);
        int width = ViewHelper.px2dip(mContext, ViewHelper.getScreenWidth(mContext) / 3);
        if(mAdapter.getData().size() == 0){
            ViewHelper.setRecycleViewLRPadding(mContext,mRecyclerView,10,10);
        } else {
            ViewHelper.setRecycleViewLRPadding(mContext,mRecyclerView,10,width);
        }
    }
